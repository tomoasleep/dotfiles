...
def configure(keymap):
    ...
    keymap_global["O-LCmd"] = eisuu
    keymap_global["O-RCmd"] = kana

    # keymap_global["Cmd-Space"] = "Alt-Space"
    keymap_global["Ctrl-OpenBracket"] = "Esc"

    def sequence(keymap, keys):
        def inner():
            for key in keys:
                if callable(key):
                    key()
    ...
    keymap_terminal["Ctrl-OpenBracket"] = sequence(keymap, ["Ctrl-OpenBracket", eisuu])
    keymap_terminal["Ctrl-C"] = sequence(keymap, ["Ctrl-C", eisuu])
    keymap_terminal["Esc"] = sequence(keymap, ["Esc", eisuu])
